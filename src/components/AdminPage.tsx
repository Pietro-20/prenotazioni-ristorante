             ...
             return (
               <button
                 ...
                 className={`px-4 py-1.5 rounded-full text-xs font-medium border transition-all ${
                    isSelected ? activeClass : 'border-gray-700 text-gray-400 hover:border-gray-500'
                 }`}
               >
                 {area}
               </button>
                      ...
                      <>
                        ...
                        <td className="px-6 py-4">
                          <span className={`text-xs font-bold uppercase tracking-wider ${
                            res.area === 'Privé Esclusivo' ? 'text-purple-400' : 
           ...
           {areas.filter(a => a !== 'Tutti').map(area => (
             (areaFilter === 'Tutti' || areaFilter === area) && (
               <div key={area} className="bg-dark-800/80 border border-white/10 rounded-2xl p-6">
                  <h3 className={`text-xl border-b border-white/5 pb-4 mb-4 flex justify-between items-center font-serif ${
                     area === 'Privé Esclusivo' ? 'text-purple-400' : 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {filteredReservations.filter(r => r.area === area).map(res => (
                      <div key={res.id} className={`p-4 rounded-xl border transition-all ${
                        editingId === res.id ? 'bg-dark-900 border-gold-500 ring-1 ring-gold-500' : 
